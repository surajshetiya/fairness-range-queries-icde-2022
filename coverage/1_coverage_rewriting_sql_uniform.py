...
def get_similarity(c, q1, q2):
    ...
    with c.cursor() as cursor:
        cursor.execute('SELECT COUNT(*) FROM uniform WHERE (' + cond1_pred[0] + ' OR ' + cond2_pred[0] + ') AND ' + '(' + cond1_pred[1] + ' OR ' + cond2_pred[1] + ') AND ' + '(' + cond1_pred[2] + ' OR ' + cond2_pred[2] + ') AND ' + '(' + cond1_pred[3] + ' OR ' + cond2_pred[3] + ') GROUP BY sa')
        union_values = cursor.fetchall()
        cursor.execute('SELECT COUNT(*) FROM uniform WHERE (' + cond1_pred[0] + ' AND ' + cond2_pred[0] + ') AND ' + '(' + cond1_pred[1] + ' AND ' + cond2_pred[1] + ') AND ' + '(' + cond1_pred[2] + ' AND ' + cond2_pred[2] + ') AND ' + '(' + cond1_pred[3] + ' AND ' + cond2_pred[3] + ') GROUP BY sa')
    ...
    for prep in ['distinct', 'qcut']:
        # for tests we read the queries from a file
         with open('uniform.txt', 'r') as file:
             for query in file:
                #query = 'SELECT * FROM uniform WHERE x > 1500 AND x < 55 AND y < 35 AND y > '
                best_sim = -1.
                best_range = None
                ...
                cursor = c.cursor()
                cursor.execute('SELECT sa,COUNT(*) FROM uniform WHERE ' + q_parts[1] + ' GROUP BY sa')
                ...
                true_weight = 1
                false_weight = 1
                delta = math.ceil((results[0][1] + results[1][1])*0.025)
                    ...
                    if disp > 0:
                        ...
                        for i in range(11):
                            ...
                            if test_result[4] != 'CC IS ALREADY SATISFIED':
                                ...
                                cursor = c.cursor()
                                cursor.execute('SELECT sa,COUNT(*) FROM uniform WHERE ' + new_query.split('WHERE')[1]  + ' GROUP BY sa')
                                results = cursor.fetchall()
                                count_dict_temp = {}
                            ...
                            if sim_val > best_sim:
                                print("Similarity:{} Range:{}".format(sim_val, new_query))
                                best_sim = sim_val
